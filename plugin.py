...
class AutonomousPlannerEventHandler(BaseEventHandler):
    ...
    handler_description = "定期检查并自动生成日程"
    weight = 10
    ...
    def __init__(self, *args, **kwargs):
        ...
        auto_schedule_config = {
            "auto_generate_daily": self.get_config("autonomous_planning.auto_schedule.auto_generate_daily", False),
            "auto_generate_weekly": self.get_config("autonomous_planning.auto_schedule.auto_generate_weekly", False),
            "auto_generate_monthly": self.get_config("autonomous_planning.auto_schedule.auto_generate_monthly", False),
            "daily_trigger_time": self.get_config("autonomous_planning.auto_schedule.daily_trigger_time", "06:00"),
            "weekly_trigger_weekday": self.get_config("autonomous_planning.auto_schedule.weekly_trigger_weekday", 0),
            "weekly_trigger_time": self.get_config("autonomous_planning.auto_schedule.weekly_trigger_time", "07:00"),
            "monthly_trigger_day": self.get_config("autonomous_planning.auto_schedule.monthly_trigger_day", 1),
            "monthly_trigger_time": self.get_config("autonomous_planning.auto_schedule.monthly_trigger_time", "08:00"),
            "use_llm_for_schedule": self.get_config("autonomous_planning.auto_schedule.use_llm_for_schedule", True),
            "preferences": self.get_config("autonomous_planning.schedule.preferences", {}),
        }
        self.auto_schedule_manager = AutoScheduleManager(
            self.goal_manager,
        ...
        self.interval = self.get_config("autonomous_planning.interval", 3600)

        # 日程检查间隔（秒），默认1小时检查一次
        self.schedule_check_interval = self.get_config("autonomous_planning.auto_schedule.check_interval", 3600)
        self.last_schedule_check = None

        logger.info(f"自主日程检查器初始化完成 (检查间隔: {self.interval}秒)")
        if auto_schedule_config.get("auto_generate_daily") or auto_schedule_config.get("auto_generate_weekly") or auto_schedule_config.get("auto_generate_monthly"):
            logger.info("✨ 自动日程生成已启用")
    ...
    async def execute(
        ...
        if not self.is_running:
            ...
            self.check_task = asyncio.create_task(self._schedule_check_loop())
            logger.info("自动日程检查循环已启动")

        return True, True, None, None, None

    async def _schedule_check_loop(self):
        """日程检查循环"""
        logger.info("🤖 麦麦自动日程系统启动")

        while self.is_running:
            try:
                await self._check_and_generate_schedule()
            except Exception as e:
                logger.error(f"日程检查异常: {e}", exc_info=True)

            # 等待下一个检查周期
            await asyncio.sleep(self.interval)

    async def _check_and_generate_schedule(self):
        """检查并生成日程"""
        now = datetime.now()

        # 检查是否需要生成日程
        if self.last_schedule_check is None or (now - self.last_schedule_check).total_seconds() >= self.schedule_check_interval:
            logger.info("开始检查是否需要自动生成日程...")
            try:
                generated = await self.auto_schedule_manager.check_and_generate()
                if generated:
                    logger.info("🎉 自动日程生成完成")
                else:
                    logger.debug("当前时间不需要生成日程")
                self.last_schedule_check = now
            except Exception as e:
                logger.error(f"自动日程生成失败: {e}", exc_info=True)
            ...
            if not goals:
                ...
                if schedule_goals:
                    ...
                    for goal in schedule_goals:
                        ...
                        if time_window:
                            start_hour = time_window[0] if len(time_window) > 0 else 0
                            end_hour = time_window[1] if len(time_window) > 1 else start_hour + 1
                            ...
                            # 简洁格式：时间 + emoji + 名称
                            messages.append(f"{start_hour:02d}:00-{end_hour:02d}:00 {type_emoji} {goal.name}")
